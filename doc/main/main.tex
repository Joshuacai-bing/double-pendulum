...
The double pendulum is an appropriate system for this purpose. It consists of two point masses connected by two rigid, massless rods, with the upper mass attached to a pivot. In the small-angle limit, its behaviour can be analysed through linear normal-mode theory; outside that limit, non-linear coupling leads to richer responses. In this project, the focus is not chaos as an isolated topic, but the broader non-linear response of a \emph{periodically driven} double pendulum, including transitions between regular and irregular motion as control parameters vary.
